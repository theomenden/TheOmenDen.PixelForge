...
/// be constructed, and the round-trip test proves what was written still validates.
/// </para>
/// <para>
...
/// schema is what replaces it, which is also why <c>RunManifest</c> copies the schema file into the
/// export directory beside the manifest it describes.
/// </para>
/// <para>
/// <b>This declaration and the schema beside it are the whole of this project.</b> It exists so the
/// generator's doc-comment diagnostics can be suppressed without costing hand-written code its
/// checks — see the csproj for why that cannot be scoped any more finely. Anything you would
/// actually write by hand belongs in Core.
/// </para>
/// </remarks>
